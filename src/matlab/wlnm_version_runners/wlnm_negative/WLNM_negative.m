function [roc_auc, pr_auc, best_threshold, best_precision, best_recall, best_f1_score, aux] = ...
    WLNM_negative(dataname, train, test, K, taxonomy, mass, role, nodeSelection, ratioTrain, varargin)
    %WLNM_NEGATIVE Baseline WLNM with negative sampling & original-style encoders.
    ...
    % === Half-matrix setup (undirected representation) ===
    htrain = triu(train, 1);
    htest  = triu(test, 1);
    Aund = spones(htrain + htrain');

    % === Negative sampling ===
    [train_pos, train_neg, test_pos, test_neg] = sample_neg_negative( ...
    ...
    % === Graph encoders ===
    [train_data, train_label] = graph2vector_negative(train_pos, train_neg, Aund, K, dataname, useParallel);
    [test_data, test_label] = graph2vector_negative(test_pos, test_neg, Aund, K, dataname, useParallel);
    ...
    net = trainNetwork(reshape(train_data', feature_dim, 1, 1, size(train_data, 1)), ...
    ...
    test_metrics = compute_binary_classification_metrics_negative(binary_predictions, test_label);
    ...
    if compute_ecological_metrics
        cmp_metrics = compare_binary_graphs_negative(empirical_full, pseudo_full);
    else
        cmp_metrics = struct();
    end
...
% ============================================================
function cmp = compare_binary_graphs_negative(empirical_full, pseudo_full)

    if isempty(empirical_full) || isempty(pseudo_full)
        error('compare_binary_graphs_negative:EmptyInput', ...
              'Both empirical_full and pseudo_full must be non-empty.');
    end

    if ~isequal(size(empirical_full), size(pseudo_full))
        error('compare_binary_graphs_negative:SizeMismatch', ...
...
function metrics = compute_binary_classification_metrics_negative(predictions, labels)
