function results = run_wlnm_negative(data, K, ratioTrain, config)
    ...
    if config.useParallel
        parfor i = 1:config.numExperiments
            results(i) = one_experiment_negative(i, dataname, train, test, K, ratioTrain, taxonomy, mass, role, nodeSelection);
        end
    else
...
function r = one_experiment_negative(i, dataname, train, test, K, ratioTrain, taxonomy, mass, role, nodeSelection)
    t0 = tic;
    disp(['Experiment ', num2str(i), ' (node selection: ', char(nodeSelection), ') - Running WLNM_negative...']);

    [auc, best_threshold, best_precision, best_recall, best_f1_score] = WLNM_negative(dataname, train, test, K, taxonomy, mass, role, nodeSelection, ratioTrain);
