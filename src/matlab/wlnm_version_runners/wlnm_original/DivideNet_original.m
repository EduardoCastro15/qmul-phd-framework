function [train, test, train_nodes, test_nodes] = DivideNet_original(net, ratioTrain, nodeSelection, use_original_logic, check_connectivity, adaptive_connectivity, rare_fraction, varargin)
    ...
    if use_original_logic
        fprintf('[DivideNet] Using WLNM original logic (legacy upper-triangular).\n');

        % Preserve the historical WLNM_original implementation exactly. It
        % projects the supplied adjacency matrix onto its upper triangle
        % before splitting and does not symmetrize the directed input first.
