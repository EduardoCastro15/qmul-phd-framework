function [train_pos, train_neg, test_pos, test_neg] = sample_neg_original(train, test, a, portion, evaluate_on_all_unseen, use_role_filter)
    ...
    pool_shortfall = max(0, need_total_requested - pool_size);
    fprintf(['[NegPool] mode=undirected_uniform pool=%d need_total=%d ' ...
        'requested_need=%d full_pool_shortfall=%d a=%g eval_all=%d ' ...
        'role_filter=%d | k_train=%d k_test=%d\n'], ...
        pool_size, need_total, need_total_requested, pool_shortfall, a, ...
        logical(evaluate_on_all_unseen), logical(use_role_filter), ...
