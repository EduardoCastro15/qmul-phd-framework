...
% run_wlnm_dir_neg, while keeping CV-specific bookkeeping fields.
    ...
    if ~isfield(config,'negativeEligibilityMode'), config.negativeEligibilityMode = 'role_only'; end
    if ~isfield(config,'negativePositiveRatio'), config.negativePositiveRatio = 2; end
    if ~isfield(config,'negativeSamplingStrategy'), config.negativeSamplingStrategy = 'uniform_without_replacement'; end
    if ~isfield(config,'negativeTopupPolicy'), config.negativeTopupPolicy = 'uniform_remaining_nonlinks'; end
    ...
    if ~isfield(config,'trophicLevelProtocol'), config.trophicLevelProtocol = 'validated_v2'; end
    if ~isfield(config,'trophicHighPrecision'), config.trophicHighPrecision = 'auto'; end
    if ~isfield(config,'trophicSnapshotDir'), config.trophicSnapshotDir = ''; end
    ...
    validate_dir_neg_trophic_scope(data.dataname, config.version, ...
        config.trophicLevelProtocol, config.trophicHighPrecision);
...
function rows = run_one_cv_experiment(e, f, k, K, ratioTrain, st, config, dataname, train, test, ...
    ...
    % WLNM_dir_neg owns the canonical eligibility protocol and controlled top-up.
    [roc_auc, pr_auc, thresholds, precisions, recalls, f1_scores, aux_rows] = WLNM_dir_neg( ...
        ...
        'model_version', get_config_text(config, 'version', 'WLNM_dir_neg_kfold'), ...
        'trophic_protocol', get_config_text(config, 'trophicLevelProtocol', 'validated_v2'), ...
        ...
        'trophic_metadata', struct('ExperimentID',e,'Seed',seed, ...
...
function out = make_cv_result_template(K, ratioTrain, cvK, foldID, expID, seed, split_stats, config)
    ...
    out = struct( ...
        ...
        'TrophicLevelProtocol',get_config_text(config, 'trophicLevelProtocol', 'validated_v2'), ...
        'NegativeEligibilityMode',get_config_text(config, 'negativeEligibilityMode', 'role_only'), ...
        'NegativePositiveRatio',get_config_number(config, 'negativePositiveRatio', 2), ...
        'NegativeSamplingStrategy',get_config_text(config, 'negativeSamplingStrategy', 'uniform_without_replacement'), ...
        'NegativeTopupPolicy',get_config_text(config, 'negativeTopupPolicy', 'uniform_remaining_nonlinks'), ...
        'RolePoolSize',NaN, 'MassPoolSize',NaN, 'EligiblePoolSize',NaN, ...
        'FullNegativePoolSize',NaN, 'RequestedNegativeCount',NaN, ...
        'SelectedNegativeCount',NaN, 'EligibleNegativeCount',NaN, ...
        'EligibleShortfall',NaN, 'FullPoolShortfall',NaN, ...
        'RandomTopupCount',NaN, 'TopupProportion',NaN, ...
        'TrainNegativeCount',NaN, 'TestNegativeCount',NaN, ...
        'CvK',cvK, ...
