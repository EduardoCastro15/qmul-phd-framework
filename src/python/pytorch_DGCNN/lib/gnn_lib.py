...
def sparse_coo_tensor(indices, values, size):
    try:
        return torch.sparse_coo_tensor(indices, values, size, check_invariants=False)
    except TypeError:
...
class _gnn_lib(object):
    ...
    def PrepareSparseMatrices(self, graph_list, is_directed=0):
        ...
        n2n_sp = sparse_coo_tensor(n2n_idxes, n2n_vals, torch.Size([total_num_nodes, total_num_nodes]))
        e2n_sp = sparse_coo_tensor(e2n_idxes, e2n_vals, torch.Size([total_num_nodes, total_num_edges * 2]))
        subg_sp = sparse_coo_tensor(subg_idxes, subg_vals, torch.Size([len(graph_list), total_num_nodes]))
