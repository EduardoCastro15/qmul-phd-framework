...
def sample_neg(net, test_ratio=0.1, train_pos=None, test_pos=None, max_train_num=None,
    ...
    if train_pos is None and test_pos is None:
        ...
        if len(row) > 1 and test_ratio > 0:
            split = min(max(split, 1), len(row) - 1)
...
def node_label(subgraph):
    ...
    labels = np.zeros(dist_to_0.shape, dtype=np.int64)
    finite_dist = np.isfinite(dist_to_0) & np.isfinite(dist_to_1)
    if np.any(finite_dist):
        dist0 = dist_to_0[finite_dist].astype(np.int64)
        dist1 = dist_to_1[finite_dist].astype(np.int64)
        d = dist0 + dist1
        d_over_2, d_mod_2 = np.divmod(d, 2)
        labels[finite_dist] = 1 + np.minimum(dist0, dist1) + d_over_2 * (d_over_2 + d_mod_2 - 1)
