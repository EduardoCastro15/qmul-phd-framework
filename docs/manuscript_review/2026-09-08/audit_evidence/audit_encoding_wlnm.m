function [data, label] = audit_encoding_wlnm(pos, neg, A, K, useParallel, dataname, use_original_wlnm)
...
function sample = subgraph2vector(ind, A, K, dataname, is_positive, idx)
    ...
    focal_i = find(order == 1); focal_j = find(order == 2);
    audit_offdiag = find(~eye(size(plweight_subgraph,1)));
    focal_vector_idx = find(audit_offdiag == sub2ind(size(plweight_subgraph),focal_i,focal_j));
    fprintf('[AUDIT] positive=%d focal_index=%d focal_value_before_eps=%.8g\n',is_positive,focal_vector_idx,full(plweight_subgraph(focal_i,focal_j)));
    % Vector encoding
